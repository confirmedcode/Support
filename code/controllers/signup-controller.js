...
(request, response, next) => {
  ...
  return SupportUser.createWithEmailAndPassword(email, password)
    .then(user => {
...
(request, response, next) => {
  ...
  return SupportUser.getWithConfirmCode(code)
    .then( user => {
      ...
      // Email not yet confirmed, confirm it.
        ...
        return SupportUser.confirmEmail(code)
          .then( success => {
...
(request, response, next) => {
  const email = request.values.email;
  SupportUser.resendConfirmCode(email)
    .then( results => {
